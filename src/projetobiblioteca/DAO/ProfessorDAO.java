...
public class ProfessorDAO implements IDAO<Professor> {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
			...
			while (linha != null) {
				...
				if (matricula == matriculaComparar) {
					return professor = new Professor(matriculaComparar, linhaSplit[1], linhaSplit[2], linhaSplit[3],
							linhaSplit[4]);
				}
	...
	public Professor buscaPorId(int matricula) {
		File file = new File(System.getProperty("user.dir") + "\\src\\projetobiblioteca\\DAO\\database\\professor.csv");
		Professor professor = null;
		...
		if (file.exists()) {
			try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
				...
				while (linha != null) {
					String[] linhaSplit = linha.split(";");
					int matriculaBanco = Integer.parseInt(linhaSplit[0]);
					String nome = linhaSplit[1];
					String endereco = linhaSplit[2];
					String dataIngresso = linhaSplit[3];
					String setor = linhaSplit[4];
					
					if (matriculaBanco == matricula) {
						professor = new Professor(matriculaBanco, nome, endereco, dataIngresso, setor);
					}
					linha = reader.readLine();
				}
			} catch (FileNotFoundException e) {
